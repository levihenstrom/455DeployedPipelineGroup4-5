...
export default function OrdersPage() {
  ...
  const filtered = orders.filter(o => {
    ...
    const matchFilter =
      ...
      filter === "ok"    ? o.is_fraud !== true && o.late_delivery !== true : true;
    ...
    <>
      ...
      <div className="grid" style={{ marginBottom: 24 }}>
        ...
        <div className="card card-red">
          ...
          <div className="value">{orders.filter((o) => o.is_fraud === true).length}</div>
        </div>
        <div className="card card-orange" style={{ borderLeftColor: "#d97706" }}>
          <div className="label">Late Deliveries</div>
          <div className="value">{orders.filter((o) => o.late_delivery === true).length}</div>
        </div>
        <div className="card card-green">
          <div className="label">All Good</div>
          <div className="value">
            {orders.filter((o) => o.is_fraud !== true && o.late_delivery !== true).length}
          </div>
        </div>
      </div>
        ...
        <div style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
          ...
          <div style={{ display: "flex", gap: 6 }}>
            {(["all", "fraud", "late", "ok"] as const).map((f) => (
              ...
              >
                {f === "all" ? "All" : f === "fraud" ? "🚨 Fraud" : f === "late" ? "⏰ Late" : "✅ OK"}
              </button>
            ))}
          </div>
        </div>
        ...
        {filtered.length === 0 ? (
          ...
          <table>
            ...
            <tbody>
              {filtered.map((o) => (
                <tr key={o.order_id}>
                  <td style={{ fontSize: 18 }}>{orderIcon(o.is_fraud, o.late_delivery)}</td>
                  ...
                  <td>
                    <Link href={`/orders/${o.order_id}`} style={{ fontSize: 13, fontWeight: 600 }}>
                      Details →
                    </Link>
                  </td>
                </tr>
