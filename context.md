...
In practice, the first version of a deployed model is often _embedded directly_

_inside an application_. A team trains a model, saves it as a file, and loads it
...
This chapter teaches that foundational deployment pattern: an _end-to-end_

_machine learning pipeline_ that lives in your application environment. You
...
sophisticated architecture—but it is _realistic_ , _teachable_ , and _extremely_

_common_ in early-stage products, internal analytics tools, and small-team
...
Save the trained model to disk as a versioned file (for example, a _.sav_
...
can run _reliably_ and _repeatably_ in a real process that other people or systems
...
realistic for small teams and many early-stage products. _This approach still_

_allows for lightweight analytical copies of the data created via scheduled_

_ETL without introducing full-scale infrastructure._
...
**Saved model file** : a serialized artifact (for example, a _.sav_ file) that the
...
1. The **application** writes new transactions and events into the

**operational database**.

2. On a schedule (for example, nightly), a **training script** reads the latest

data from the database.

3. The script runs **the same cleaning and feature engineering logic** you

used during modeling, rather than re-implementing transformations
...
4. The script trains and evaluates a model, then saves the **model artifact**

(and often a preprocessing artifact) to a known location.

5. The **application** loads the newest saved model and uses it to generate

predictions during normal workflows.
...
## 17.3End-to-End Pipeline

An end-to-end machine learning deployment pipeline is simply a structured

way to turn the CRISP-DM process into executable code. Every stage you

learned earlier—understanding data, preparing it, modeling, and evaluation

—still exists. The difference is that these steps now run automatically and

repeatedly.

Seen this way, deployment is not a new discipline. It is _CRISP-DM_
...
preprocessing objects) to disk in a standardized format such as _.sav_. This
...
We begin by connecting to the operational database and loading the tables

needed for predicting late delivery.
...
At this stage, the data is highly normalized and not suitable for modeling.

Our goal is to collapse these tables into one row per order.
...
Next, we join customers, orders, and aggregated item features into a single
...
We now create a small number of transparent features that plausibly

influence delivery delays. These features are deterministic and reproducible,

making them safe to use in both training and inference.
...
Notice that no model logic appears here. This script constructs features and

labels only; learning happens later.

For this chapter, we predict whether an order is delivered late using a simple
...
The variable _delivery_days_ is used only to construct the label and is

intentionally excluded from modeling features to avoid data leakage.
...
Once created, _warehouse.db_ becomes the sole data source for training and

evaluation, forming a clear contract between data engineering and modeling.

This mirrors real deployment pipelines: operational systems feed analytics

systems, which feed models.
...
We explicitly define the columns used for modeling. This makes the pipeline

transparent and ensures the same features are used during both training and
...
Notice what is missing here: we do not perform ad-hoc cleaning or feature

creation inside the training step. That work belongs in ETL so that training

remains consistent, repeatable, and auditable.

We now introduce scikit-learn’s _Pipeline_ object. A pipeline packages

preprocessing and the model into a single unit that can be saved to disk and

reused during inference.

This is a deployment best practice: the exact same transformations must be

applied at training time and prediction time.
...
Training becomes a single method call. The pipeline learns both

preprocessing parameters (imputation values and scaling parameters) and the
...
Before saving the model, evaluate on unseen data. Even in deployment

contexts, this remains standard supervised learning: test sets still matter.
...
model so performance can be compared across retraining runs.

A trained model is just a file. We serialize the entire pipeline using _joblib_ ,
...
This single file can now be loaded by any Python process for inference. That

is why we say: _Your model is a file._
...
Production models should always be accompanied by metadata (training

context) and metrics (evaluation results). These files make the model

auditable, traceable, and reproducible.
...
prediction. This phase is called _inference_.

Inference does not involve training, gradients, or optimization. The model is

loaded from disk and used to generate predictions on new data.
...
Predictions are generated on live operational data, not the analytical
...
Inference must apply the _same feature logic_ used during training. In

production systems, this logic should be shared code imported by both
...
This step reinforces why pipelines and shared feature logic are critical in
...
Probabilities are often more useful than binary predictions for operational

decision-making.

Predictions become most valuable when written back into systems that drive
...
At this point, the web application does not need to know anything about
...
This job loads the latest saved model and generates predictions for

unfulfilled orders. It then writes predictions to a dedicated table keyed by
...
feature. Here, that feature is a warehouse _priority queue_ : orders with the
...
This query assumes you have a table named _order_predictions_ keyed by

_order_id_ with columns _late_delivery_probability_ , _predicted_late_delivery_ ,

and _prediction_timestamp_.
...
If you have little or no web development background, use the _JavaScript_

_stack_ (Next.js) for the most guided path.

If you are strongest in Python, use the _Python stack_ (FastAPI) to keep

everything in one language.

If you have C# experience, use the _ASP.NET stack_ (minimal APIs or
...
predictions into _order_predictions_ so the app can display this priority queue.
...
In this section, you will _vibe code_ a complete (but simple) web app on top of
...
database tables you already have (for example, _customers_ , _orders_ ,

_order_items_ , _products_ , and _order_predictions_ ). If your database uses different

table or column names, update the prompts below to match your schema.

The pipeline writes predictions into _order_predictions_ keyed by _order_id_.

The application should treat that table like any other application table.

For students with limited background, use: _Next.js + SQLite_ for the web app,

and a separate _Python inference script_ that writes predictions into the
...
- A small table of the 5 most recent orders (order_id, order_timestamp, fulfilled,
total_value)
...
- quantity
3. On submit:
- Insert a row into orders for this customer with fulfilled = 0 and
order_timestamp = current time
...
_order_predictions_. The app then reloads the priority queue.
...
pipeline to predict _late delivery_ and integrate those predictions directly into
...
_is_fraud_ : a binary classification task focused on identifying potentially

fraudulent orders.

_risk_score_ : a regression task that estimates overall order risk on a
...
Build a _separate pipeline_ with its own ETL script, training script, and

model artifact.

Extend your existing pipeline to support _multiple targets_ using shared
...
_order_predictions_fraud_ or _order_predictions_risk_ , and include a prediction

timestamp column.

For _is_fraud_ , accuracy can be misleading, so include at least precision, recall,

and F1 (or PR AUC if you know how). For _risk_score_ , include at least MAE
...
_Fraud prediction_ : flag orders for manual review, delayed fulfillment, or

additional verification.

_Risk score_ : sort or filter orders by risk level, or combine risk with
...
Share links to AI chat conversations (e.g., ChatGPT, Claude, Cursor chats) that show how you used AI tools to complete this project. You do not need to fill all 8 slots—submit only as many as you have. Even 1–2 links are fine.

Points: 20
Resource Files:1 file
File Icon
shop.db
Your Submission:Started on Mar 31 at 12:50
Bookmark question for later
Part 1: Vercel Deployment URL

Paste the URL to your deployed Vercel project below. This should be the live web app you built following the steps in Chapter 17 (Sections 17.1–17.9), backed by the shop.db database.

Bookmark question for later
Part 2: CRISP-DM Jupyter Notebook — Predicting is_fraud

Upload your Jupyter notebook (.ipynb file) that builds a complete ML pipeline predicting the is_fraud column in the orders table of shop.db.

Your notebook should demonstrate the full CRISP-DM process using the skills learned in Chapters 1–16, including: business understanding, data understanding and exploration (Ch. 6, 8), data preparation and wrangling (Ch. 2–4, 7), classification modeling (Ch. 13), ensemble methods (Ch. 14), model evaluation, selection, and tuning (Ch. 15), feature selection (Ch. 16), and model serialization for deployment (Ch. 17).

1		
Please upload 1 file(s).
Bookmark question for later
Part 3: AI Chat Log URL #1 (Optional)

Paste a shared link to an AI chat conversation (e.g., ChatGPT, Claude, Cursor) that you used while completing this project. You do not need to fill all 8 chat URL fields — submit only as many as you have. Even 1–2 is fine. Leave blank if not applicable.

Bookmark question for later
AI Chat Log URL #2 (Optional)

Paste another shared AI chat link, or leave blank.

Bookmark question for later
AI Chat Log URL #3 (Optional)

Paste another shared AI chat link, or leave blank.

Bookmark question for later
AI Chat Log URL #4 (Optional)

Paste another shared AI chat link, or leave blank.

Bookmark question for later
AI Chat Log URL #5 (Optional)

Paste another shared AI chat link, or leave blank.

Bookmark question for later
AI Chat Log URL #6 (Optional)

Paste another shared AI chat link, or leave blank.

Bookmark question for later
AI Chat Log URL #7 (Optional)

Paste another shared AI chat link, or leave blank.

Bookmark question for later
AI Chat Log URL #8 (Optional)
